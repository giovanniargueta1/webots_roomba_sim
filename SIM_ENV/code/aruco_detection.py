#
# Python controller that handles two balls and two markers sequentially
...
CENTER_TOL     = 15.0        # px tolerance for centering
TURN_SPEED     = 1.0         # wheel velocity when turning
FORWARD_SPEED  = 3.5         # wheel velocity when driving forward
...
BACKUP_TIME    = 30          # Time steps to back up (30 * 64ms = ~2 seconds)
    ...
    while robot.step(TIME_STEP) != -1:
        ...
        if current_state in [State.DRIBBLE_FIND_MARKER, State.DRIBBLE_ALIGN_MARKER, State.DRIBBLE_TO_MARKER,
            ...
            if ids is not None and len(ids) > 0:
                ...
                if marker_id is not None and marker_corners is not None:
        ...
        if current_state == State.FIND_BALL:
            ...
            if ball_x is None:
                ...
                if abs(ball_offset_x) > CENTER_TOL:
                    turn = TURN_SPEED if ball_offset_x > 0 else -TURN_SPEED
                    left_spd, right_spd = turn, -turn
                    action = f"Aligning with ball (offset={ball_offset_x:.1f}px)"
                else:
                    # Ball is centered, move to approach
            ...
            if ball_x is None:
                ...
                ball_offset_x = ball_x - (width / 2.0)
                if abs(ball_offset_x) > CENTER_TOL * 1.5:  # Wider tolerance during approach
                    # Need to realign
                    current_state = State.ALIGN_WITH_BALL
                    ...
                    left_spd = right_spd = SLOW_SPEED
                    action = f"Approaching ball (d≈{ball_distance:.2f}m)"
            ...
            if marker_id is None:
                ...
                if abs(marker_offset_x) > CENTER_TOL:
                    turn = TURN_SPEED * 0.8 if marker_offset_x > 0 else -TURN_SPEED * 0.8
                    left_spd, right_spd = turn, -turn
                    action = f"Aligning with marker (offset={marker_offset_x:.1f}px)"
                else:
                    # Marker is centered, move to approach
            ...
            if marker_id is None:
                ...
                if abs(marker_offset_x) > CENTER_TOL * 1.5:  # Wider tolerance during approach
                    # Need to realign
                    current_state = State.DRIBBLE_ALIGN_MARKER
                    ...
                    first_marker_processed = True  # Flag that we've processed the first marker
                    action = "First marker reached! Preparing for second ball"
                else:
                    # Move toward the marker with the ball
                    left_spd = right_spd = FORWARD_SPEED * 1
                    action = f"Approaching marker with ball (d≈{marker_distance:.2f}m)"
            ...
            if backup_counter > 0:
                ...
                backup_counter -= 1
                left_spd = -SLOW_SPEED * 0.5  # Slower backing up
                right_spd = -SLOW_SPEED * 2.0  # Turn right more aggressively while backing up
        ...
        # Second ball and marker states - similar logic to first ball/marker
            ...
            if ball_x is None:
                ...
                if abs(ball_offset_x) > CENTER_TOL:
                    turn = TURN_SPEED if ball_offset_x > 0 else -TURN_SPEED
                    left_spd, right_spd = turn, -turn
                    action = f"Aligning with second ball (offset={ball_offset_x:.1f}px)"
                else:
                    # Ball is centered, move to approach
            ...
            if ball_x is None:
                ...
                if abs(ball_offset_x) > CENTER_TOL * 1.5:  # Wider tolerance during approach
                    ...
                    # Move toward the ball
                    left_spd = right_spd = SLOW_SPEED
                    action = f"Approaching second ball (d≈{ball_distance:.2f}m)"
            ...
            if marker_id is None:
                ...
                if abs(marker_offset_x) > CENTER_TOL:
                    turn = TURN_SPEED * 0.8 if marker_offset_x > 0 else -TURN_SPEED * 0.8
                    left_spd, right_spd = turn, -turn
                    action = f"Aligning with second marker (offset={marker_offset_x:.1f}px)"
                else:
                    # Marker is centered, move to approach
            ...
            if marker_id is None:
                ...
                if abs(marker_offset_x) > CENTER_TOL * 1.5:  # Wider tolerance during approach
                    ...
                    # Move toward the marker with the ball
                    left_spd = right_spd = FORWARD_SPEED * 1
                    action = f"Approaching second marker with ball (d≈{marker_distance:.2f}m)"
        ...
        print(f"State: {current_state.name} | Ball: pos={b_pos}, r={b_rad}, d={b_dist} | "
